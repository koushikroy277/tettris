...
bg_sound = pygame.mixer.music.load('bensound-clearday.wav')
...
SHAPES = [S, Z, I, O, J, L, T]
shape_color = [(0, 255, 0), (255, 0, 0), (0, 255, 255),
            (255, 255, 0), (255, 120, 0), (0, 0, 255),
            (110, 0, 110)]
...
def draw_text_middle(surface, text, size, color):
    font = pygame.font.SysFont('comicsans', size, bold=True, italic=False)
...
def draw_grid(surface, row, col):
    ...
    for i in range(row):
        pygame.draw.line(surface, (120, 120, 120),
            (sx, sy + i * block_size), (sx * 2.2, sy + i * block_size))
        for j in range(col):
            pygame.draw.line(surface, (120, 120, 120),
...
def draw_window(surface, grid, score, last_score):
    ...
    surface.blit(label, (sx + 30, sy + 165))
...
def main(win):
    ...
    while run:
        ...
        if check_lost(locked_positions):
            draw_text_middle(win, "You lost the game!!!", 40, (255, 255, 255))
            update_score(score)
            run = False
...
def main_menu(win):
    ...
    while run:
        ...
        win.fill((0, 0, 0))
        draw_text_middle(win, "Press any key to continue", 60, (255, 255, 255))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
